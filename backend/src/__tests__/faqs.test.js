jest.mock('../utils/email', () => {
  const noop = jest.fn().mockResolvedValue(true);
  return {
    sendEmail: noop,
    sendPasswordResetEmail: noop,
    sendBookingConfirmation: noop,
    sendAdminNotification: noop,
    sendBookingConfirmed: noop,
    sendQuoteNotification: noop,
    sendQuoteConfirmation: noop,
    sendQuoteStatusUpdate: noop,
    sendRegistrationAdminNotification: noop,
    sendRegistrationConfirmed: noop,
    sendContactNotification: noop,
  };
});

const { mockModel, mockQuery } = require('./helpers/mockFactory');
const { createAuthToken } = require('./helpers/createAdmin');
const { validFAQ } = require('./helpers/factories');

const mockAdminDoc = {
  _id: '507f1f77bcf86cd799439011',
  name: 'Admin User',
  email: 'admin@example.com',
  role: 'superadmin',
  comparePassword: jest.fn().mockResolvedValue(true),
  getSignedJwtToken: jest.fn().mockReturnValue('jwt-token'),
  getSignedRefreshToken: jest.fn().mockReturnValue('refresh-token'),
  save: jest.fn().mockResolvedValue(undefined),
  toObject: () => ({
    _id: '507f1f77bcf86cd799439011',
    name: 'Admin User',
    email: 'admin@example.com',
    role: 'superadmin',
  }),
};

jest.mock('../models/Admin', () => mockModel({
  findById: jest.fn().mockReturnValue(mockQuery(mockAdminDoc)),
}));

const mockFAQs = [
  { _id: '507f1f77bcf86cd799439011', question: 'Q1?', answer: 'A1', order: 1 },
  { _id: '507f1f77bcf86cd799439012', question: 'Q2?', answer: 'A2', order: 2 },
];

jest.mock('../models/FAQ', () => mockModel({
  find: jest.fn().mockReturnValue(mockQuery(mockFAQs)),
  findById: jest.fn().mockReturnValue(mockQuery(mockFAQs[0])),
  findByIdAndUpdate: jest.fn().mockImplementation((id, update) =>
    Promise.resolve({ _id: id, ...mockFAQs[0], ...update, toObject: () => ({ ...mockFAQs[0], ...update }) })
  ),
  findByIdAndDelete: jest.fn().mockResolvedValue(mockFAQs[0]),
  insertMany: jest.fn().mockResolvedValue(mockFAQs),
}));

const request = require('supertest');
const app = require('../app');

describe('FAQs API', () => {
  afterEach(() => {
    jest.clearAllMocks();
  });

  describe('GET /api/faqs', () => {
    test('returns all FAQs sorted by order', async () => {
      const res = await request(app).get('/api/faqs');
      expect(res.status).toBe(200);
      expect(res.body.success).toBe(true);
      expect(Array.isArray(res.body.data)).toBe(true);
      expect(res.body.data.length).toBe(2);
    });
  });

  describe('POST /api/faqs', () => {
    test('creates FAQ when authenticated', async () => {
      const token = createAuthToken();
      const res = await request(app)
        .post('/api/faqs')
        .set('Authorization', `Bearer ${token}`)
        .send(validFAQ());
      expect(res.status).toBe(201);
      expect(res.body.success).toBe(true);
      expect(res.body.data).toBeDefined();
    });

    test('rejects without auth', async () => {
      const res = await request(app)
        .post('/api/faqs')
        .send(validFAQ());
      expect(res.status).toBe(401);
    });

    test('validates required fields', async () => {
      const token = createAuthToken();
      const res = await request(app)
        .post('/api/faqs')
        .set('Authorization', `Bearer ${token}`)
        .send({ question: 'Only question?' });
      expect(res.status).toBe(400);
    });
  });

  describe('PUT /api/faqs/:id', () => {
    test('updates FAQ when authenticated', async () => {
      const token = createAuthToken();
      const res = await request(app)
        .put('/api/faqs/507f1f77bcf86cd799439011')
        .set('Authorization', `Bearer ${token}`)
        .send({ answer: 'Updated answer' });
      expect(res.status).toBe(200);
      expect(res.body.success).toBe(true);
    });

    test('rejects without auth', async () => {
      const res = await request(app)
        .put('/api/faqs/507f1f77bcf86cd799439011')
        .send({ answer: 'Updated answer' });
      expect(res.status).toBe(401);
    });
  });

  describe('DELETE /api/faqs/:id', () => {
    test('deletes FAQ when authenticated', async () => {
      const token = createAuthToken();
      const res = await request(app)
        .delete('/api/faqs/507f1f77bcf86cd799439011')
        .set('Authorization', `Bearer ${token}`);
      expect(res.status).toBe(200);
      expect(res.body.success).toBe(true);
    });

    test('rejects without auth', async () => {
      const res = await request(app)
        .delete('/api/faqs/507f1f77bcf86cd799439011');
      expect(res.status).toBe(401);
    });
  });

  describe('PUT /api/faqs/reorder', () => {
    test('reorders FAQs when authenticated', async () => {
      const token = createAuthToken();
      const res = await request(app)
        .put('/api/faqs/reorder')
        .set('Authorization', `Bearer ${token}`)
        .send({ orders: [
          { id: '507f1f77bcf86cd799439011', order: 2 },
          { id: '507f1f77bcf86cd799439012', order: 1 },
        ] });
      expect(res.status).toBe(200);
      expect(res.body.success).toBe(true);
    });

    test('rejects without auth', async () => {
      const res = await request(app)
        .put('/api/faqs/reorder')
        .send({ orders: [{ id: '507f1f77bcf86cd799439011', order: 1 }] });
      expect(res.status).toBe(401);
    });

    test('validates orders array', async () => {
      const token = createAuthToken();
      const res = await request(app)
        .put('/api/faqs/reorder')
        .set('Authorization', `Bearer ${token}`)
        .send({ orders: 'not-an-array' });
      expect(res.status).toBe(400);
    });
  });
});
